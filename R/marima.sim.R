...
##' typically generated when applying the function define.dif. May be
...
##' Model2   <- define.model(kvar=7, ar=c(1), ma=c(1), rem.var=c(1,6,7),
##' indep=NULL)
...
marima.sim <- function(kvar=1, ar.model=NULL, ar.dif=NULL, ma.model=NULL, 
                       averages=rep(0 , kvar), resid.cov = diag(kvar), 
                       seed=4711, nstart=0, nsim=0 ) {
...
for ( i in 1:kvar ) { y[ , i] <- y[ , i] + averages[i] }
